...
class AdjacencyList:
    ...
    def add_node(self, name, info=None):
        ...
        Returns an adjacency list head.
        '''
        # Empty list: turn this cell into the new node.
        if self.is_empty():
            self._name = name
            self._info = info
            self._tail = AdjacencyList()  # empty tail
            self._edges = Edge()          # empty edge list
            return self.head()

        # Node already exists: update info.
        if name == self.name():
            self._info = info
            return self.head()
        ...
        if name < self.name():
            new_head = AdjacencyList(name, info)
            # Attach current list as tail of new head.
            new_head.cons(self.head())
            return new_head.head()

        # Insert into tail.
        self._tail = self.tail().add_node(name, info)
        return self.head()
    ...
    def delete_node(self, name):
        ...
        # Since list is ordered, if name is smaller it cannot exist.
        if name < self.name():
            return self.head()

        self._tail = self.tail().delete_node(name)
        return self.head()
    ...
    def _add_edge(self, src, dst, weight):
        ...
        if src == self.name():
            self._edges = self.edges().add(dst, weight)
            return self.head()

        if src < self.name():
            return self.head()

        self._tail = self.tail()._add_edge(src, dst, weight)
        return self.head()
    ...
    def delete_edge(self, src, dst):
        ...
        if src == self.name():
            self._edges = self.edges().delete(dst)
            return self.head()

        if src < self.name():
            return self.head()

        self._tail = self.tail().delete_edge(src, dst)
        return self.head()
    ...
    def delete_edges(self, name):
        ...
        self._edges = self.edges().delete(name)
        self._tail = self.tail().delete_edges(name)
        return self.head()
    ...
    def find_edge(self, src, dst):
        ...
        # Quick fail if src/dst are not nodes.
    ...
    def self_loops(self):
        ...
        if self.is_empty():
            return 0
        count = 1 if self.edges().find(self.name()) else 0
    ...
    def adjacency_matrix(self):
        ...
        for (src, dst, w) in self.list_edges():
...
class Edge:
    ...
    def add(self, dst, weight=1):
        ...
        Returns an edge head.
        '''
        # Empty list: turn this cell into the new edge.
        if self.is_empty():
            self._dst = dst
            self._weight = weight
            self._tail = Edge()
        ...
        # Insert before current head.
        if dst < self.dst():
            new_head = Edge(dst, weight)
            new_head.cons(self.head())
            return new_head.head()

        # Insert into tail.
        self._tail = self.tail().add(dst, weight)
        return self.head()
    ...
    def delete(self, dst):
        ...
        # Lexicographical order allows early stop.
        if dst < self.dst():
            return self.head()

        self._tail = self.tail().delete(dst)
        return self.head()
