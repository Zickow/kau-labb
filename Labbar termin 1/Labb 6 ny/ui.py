...
class TerminalUI:
    ...
    def show_2d(self):
        ...
        '''
        bfs = self._tree.bfs_order_star()
        bfs = [v if v is not None else "*" for v in bfs]
        h = self._tree.height()
        if h == 0:
            return

        # Print each level with spacing that scales with the height.
        idx = 0
        for level in range(h):
            nodes_in_level = 2 ** level
            gap = 2 ** (h - level)  # proportional spacing
